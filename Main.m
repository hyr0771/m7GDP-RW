...
clear all %#ok<CLALL>
% load data
m7g_disease_data = importdata('.\datasets\m7g_disease_data');
Wmm = importdata('.\datasets\m7GSim');
Wdd = importdata('.\datasets\DiseaseSim'); 
share_mm = importdata('.\datasets\share_mm'); 
share_dd = importdata('.\datasets\share_dd'); 
Wrname = importdata('.\datasets\m7gName'); 
Wdname = importdata('.\datasets\DiseasesName');
...
Score = RWR(Wmd_train,LWmm,LWdd,alpha);
Rt{h} = Score;
